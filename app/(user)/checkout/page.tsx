...
export default function CheckoutPage() {
  ...
  const searchAreas = async (value: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/biteship/maps?input=${encodeURIComponent(value)}`, {
        cache: "no-store",
      });
  ...
  const handleCheckout = () => {
    ...
    if (!address.name || !address.phone || !address.address || !address.detailAddress || !address.detailOther) {
      setOpen(true);
      return;
    }
    ...
    sendToWhatsApp({
      items,
      totalPrice: totalPrice + selectedRate.price,
      address,
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            {!loadingRates && address.address && rates.length === 0 && (
              <p className="text-sm text-muted-foreground">Ongkir belum tersedia.</p>
            )}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-medium">{selectedRate ? formatPrice(selectedRate.price) : "-"}</span>
            </div>
            ...
            <div className="flex items-center justify-between border-t pt-4">
              ...
              <span className="text-lg font-bold text-primary">
                {formatPrice(totalPrice + (selectedRate?.price ?? 0))}
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <DrawerContent className="max-w-md mx-auto">
          ...
          <div className="space-y-4 px-4 pb-4 overflow-y-auto max-h-[70vh]">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                rows={4}
                value={detailAddress}
                placeholder="Cth. Jl. Nama Jalan No. 123, RT 01/RW 02, Kelurahan, Kecamatan, Kota"
                onChange={(e) => setDetailAddress(e.target.value)}
              />
            </div>

            <div className="space-y-2">
              <Label htmlFor="detail-address">Detail Lainnya (Cth: Blok / Unit No., Patokan)</Label>

              <Textarea
                id="detail-other"
                rows={4}
                value={detailOther}
                placeholder="Cth: Blok C No. 12, cat rumah warna krem, patokan masuk gang sebelah apotek."
          ...
          <DrawerFooter className="border-t bg-background">
            <Button
              ...
              disabled={
                !name.trim() || !phone.trim() || !keyword.trim() || !detailAddress.trim() || !detailOther.trim()
              }
            >
              Simpan Alamat
            </Button>
